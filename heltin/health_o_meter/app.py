...
health_bp = Blueprint('health', __name__, template_folder='templates', static_folder='static')
...
@health_bp.route('/profile', methods=['GET', 'POST'])
...
def profile():
    if request.method == 'POST':
        ...
        return redirect(url_for('health.upload_nutrition'))  # Updated to include blueprint name
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                return redirect(url_for('health.results'))  # Updated to include blueprint name
            except Exception as e:
                return jsonify({'error': str(e)}), 500
...
def results():
    # Check if we have all required data
    if 'user_profile' not in session or 'nutrition_info' not in session:
        return redirect(url_for('health.index'))  # Updated to include blueprint name
